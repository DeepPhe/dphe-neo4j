...
//      ShutdownHook.registerShutdownHook( graphDb );
    ...
    public void addPatientInfo(final GraphDatabaseService graphDb,
        ...
        try (Transaction tx = graphDb.beginTx()) {
            ...
            String nameValue = patient.getName();
            if ( nameValue== null || nameValue.isEmpty())
                nameValue = "John Smith";

            String genderValue = patient.getGender();
            if (genderValue == null || genderValue.isEmpty())
                genderValue = "M";

            String birthValue = patient.getBirth();
            if (birthValue == null || birthValue.isEmpty())
                birthValue = "01/01/1970";

            String deathValue = patient.getDeath();
            if (deathValue == null || deathValue.isEmpty())
                deathValue = "N";

            String firstEncounterValue = "01/01/1990"; //getFirstDateSlashText()
            String lastEncounterValue = "06/06/2000"; //getLastDateSlashTest()


            patientNode.setProperty(adjustPropertyName(PATIENT_NAME), nameValue);
            patientNode.setProperty(adjustPropertyName(PATIENT_GENDER), genderValue);
            patientNode.setProperty(adjustPropertyName(PATIENT_BIRTH_DATE), birthValue);
            patientNode.setProperty(adjustPropertyName(PATIENT_DEATH_DATE), deathValue);
            patientNode.setProperty(adjustPropertyName(PATIENT_FIRST_ENCOUNTER), firstEncounterValue);
            patientNode.setProperty(adjustPropertyName(PATIENT_LAST_ENCOUNTER), lastEncounterValue);
...
//         node.setProperty( adjustPropertyName( PATIENT_LAST_ENCOUNTER ), patient.getLastDateSlashText() );
    ...
    /***
     * 2021-03-19: Created this as a workaround until the patient information is populated.
     * @param graphDb
     * @param log
     * @param patient
     */
    public void addPatientInfoForTesting(final GraphDatabaseService graphDb,
                               final Log log,
                               final Patient patient) {
        try (Transaction tx = graphDb.beginTx()) {
            final Node patientNode = createPatientNode(graphDb, log, patient.getId());
            ...
            patientNode.setProperty( adjustPropertyName( PATIENT_NAME ), patient.getName() );
            patientNode.setProperty( adjustPropertyName( PATIENT_GENDER ), patient.getGender() );
            patientNode.setProperty( adjustPropertyName( PATIENT_BIRTH_DATE ), patient.getBirth() );
            patientNode.setProperty( adjustPropertyName( PATIENT_DEATH_DATE ), patient.getDeath() );
            // TODO : Dynamic
//         node.setProperty( adjustPropertyName( PATIENT_FIRST_ENCOUNTER ), patient.getFirstDateSlashText() );
//         node.setProperty( adjustPropertyName( PATIENT_LAST_ENCOUNTER ), patient.getLastDateSlashText() );
            patient.getNotes().forEach(n -> addNoteInfo(graphDb, log, patientNode, n));
...
//         node.setProperty( NOTE_NAME, note.getId() );
    ...
    static private void addMentionInfo(final GraphDatabaseService graphDb,
                                       ...
                                       final Node noteNode,
                                       final Mention mention) {
        //2021-03-17: John and Sean discovered that if we try to add a node that is already in the neo4j database, the
        //            transaction will immediately fail when executed.  First attempt to "fix" this will be to simply
        //            "not" try to overwrite anything.  Second attempt will likely perform "updates" properly.

        final Node sameNode = graphDb.findNode(TEXT_MENTION_LABEL, NAME_KEY, mention.getId());
        if (sameNode == null) {
            try (Transaction tx = graphDb.beginTx()) {
                //03-17   final Node sameNode = graphDb.findNode( TEXT_MENTION_LABEL, NAME_KEY, mention.getId());
                //before 03-17 final Node sameNode = graphDb.findNode( TEXT_MENTION_LABEL, NAME_KEY, mention._id );
                //before 03-17  if ( sameNode != null ) {
                //before 03-17             annotationNodes.put( annotation, sameNode );
                //before 03-17            tx.success();
                //before 03-17            return sameNode;
                //before 03-17         }
...
//         annotationNodes.put( annotation, node );
                ...
                log.error("Ignoring Exception " + e.getMessage());
                // Attempt to continue.
                System.out.println("NodeWriter.addMentionInfo.1:" + e.getMessage());
            }
        } else {
            //if this else is triggered, the node that we planned on adding already existed, and as of 2021-03-17, that
            //means we didn't try to add it...let's throw an error (but let's not stop execution over it)

            String message = "addMentionInfo was passed a mention.getId() that already exists in the database.";
            log.error("Ignoring Exception " + message);
            System.out.println("NodeWriter.addMentionInfo.2:" + message);
        }
    }
